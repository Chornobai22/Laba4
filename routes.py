...
def update_rating(id):
    ...
    check_accessusers = s.query(User).filter(User.id == request.json.get('user_creator_id')).first()
    if check_accessusers.accessusers != 'Admin':
        return {"message": "This user is not an Admin."}, 406 #стоворить тільки адмін і
    # тільки він може змінити цей запис тому перевірка на не адімна немає сенсу
    s.commit()
    return {"message": "Rating was successfully updated. "}, 200


@query.route('/g/<int:id>', methods=['DELETE'])
@jwt_required()
...
def register_user():
    ...
    return {'access_token': token, 'new_user_id': new_user.id, "new_user_username" : new_user.username}, 200
...
def get_user(user_id):
    ...
    if user.id != logged_in_user_id:
        return {'message': 'Access denied'}, 403
    serialized = {"id": user.id,"name": user.name,"surname": user.surname,"username": user.username,"accessusers": user.accessusers}
